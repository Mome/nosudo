#!/usr/bin/env bash
# End-to-end test of nosudo inside the VM. Assumes `vm/up.sh` has run.
#
...
#                        Persistent=true fires the restore on boot. nosudo is
...
DENY=/etc/sudoers.d/zz-nosudo-tester
TIMER=nosudo-restore-tester.timer
...
r_ok "restrict tester --for 30m"                "$NOSUDO restrict tester --for 30m"
...
r_ok "restore tester"                           "$NOSUDO restore tester"
...
r_ok "restore after reboot"                     "$NOSUDO restore tester"
...
r_ok "restrict tester --until $T"               "$NOSUDO restrict tester --until $T"
r_ok "deny present before downtime"             "test -f $DENY"
r_ok "restore timer armed"                       "$ARMED"
echo "  .. uninstalling nosudo to prove restore is self-contained"
vm_ssh "$UV tool uninstall nosudo" >/dev/null 2>&1 || true
u_no "nosudo is gone"                            "test -x $NOSUDO"
...
[ "$restored" -eq 0 ] && ok "deny removed on boot by Persistent timer (nosudo absent)" \
